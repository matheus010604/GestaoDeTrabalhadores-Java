...
public class AVL {
                ...
                if (flagInsercao) { //Se já inseriu
                    switch (raiz.getFb()) {
                        ...
                        case -1: //Caso já tinha um filho esquerdo, tem que rotacionar
                            //Se o filho esquerdo tinha um filho esquerdo, então rotação simples para a direita
                ...
                if (flagInsercao) { //Se já inseriu
                    switch (raiz.getFb()) {
                        ...
                        case 1: //Se jã tinha filhos direito, tem que rotacionar
                            //Se o filho direito tiver um filho direito, então é rotação simples para a esquerda
    ...
    public String emOrdemStringInvertido() {
        return emOrdemStringInvertido(raiz);
    }

    private String emOrdemStringInvertido(NoAVL raiz) {
        String resp = "";
        if (raiz != null) {
            resp += emOrdemStringInvertido(raiz.getDir());
            resp += raiz.getDado() + ", \n";
            resp += emOrdemStringInvertido(raiz.getEsq());
        }
    ...
    public int count(NoAVL raiz) {
        int total = 0;
        if (raiz != null) {
            total += count(raiz.getEsq());
            total += count(raiz.getDir());
            total++; // total = total + 1;
        }
        return total;
    }
